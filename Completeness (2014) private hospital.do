...
* Completeness assessment private hospitals EFY 2014 (2021-22)

*------------------------------------------------------------------------------*
* Note: before importing the data from csv file i have added one variable (orgunitlevel5) in the csv file so that i can get equal number of variables (83 variables) with the same name when I import it to Stata. 
...
*note: since there is not private hospital in somali region i have droped somali region from the data
drop if region==3
...
export excel using "$project/completeness_private hospital 2014.xlsx", sheet(Option_two) firstrow(variable) sheetreplace
